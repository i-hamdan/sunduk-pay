...
public class TransferService implements WalletOperation {
    ...
    public MainWalletResponse perform(final MainWalletRequest mainWalletRequest) {
        log.info("Processing transfer request for UUID: {}, Request: {}", mainWalletRequest.getUuid(), mainWalletRequest);
        ...
        if (sourceExists && targetExists) {
            ...
            return handleInternalTransfer(user, mainWallet, mainWalletRequest.getAmount(),
                    sourceWallet, targetWallet, sourceWallet.getBalance(), targetWallet.getBalance());
    ...
    private boolean isExternalPayment(final MainWalletRequest request) {
        return request.getPaymentMethod() == PaymentMethod.UPI || request.getPaymentMethod() == PaymentMethod.BANK;
    }

    private MainWalletResponse handleInternalTransfer(final User user, final MainWallet mainWallet,
                                                      final Double amount, final WalletWrapper sourceWallet,
                                                      final WalletWrapper targetWallet,
                                                      final Double previousSourceBalance,
                                                      final Double previousTargetBalance) {
        return internalTransferService.doInternalTransfer(user, mainWallet, amount,
                sourceWallet, targetWallet, previousSourceBalance, previousTargetBalance);
    }

    private MainWalletResponse handleExternalOutGoingTransfer(final WalletWrapper sourceWallet,
                                                              final WalletWrapper targetWallet,
        ...
        return paymentService.createCheckoutSession(user.getUuid(), amount, TransactionType.DEBIT, targetWallet, sourceWallet);
    }

    private MainWalletResponse handleExternalIncomingTransfer(final User user, final Double amount,
                                                              final WalletWrapper targetWallet,
