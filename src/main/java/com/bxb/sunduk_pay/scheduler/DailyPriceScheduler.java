...
/**
 * Scheduled component responsible for running the daily asset price fetching job.
 * It fetches the latest closing price and ensures that the price for a specific
 * date is only persisted once (idempotency) using SLF4J logging.
 *
 */
@Component
public class DailyPriceScheduler {
    ...
    public DailyPriceScheduler(AssetRepository assetRepository, AssetPriceRepository assetPriceRepository) {
        this.assetRepository = assetRepository;
        this.assetPriceRepository = assetPriceRepository;
    }

    /**
     * Scheduled job to fetch the latest closing price for all configured assets.
     * The cron expression "0 0 17 * * MON-FRI" runs at 5:00 PM (17:00) every weekday.
     */
    ...
    public void runDailyPriceFetch() {
        ...
        logger.info("Daily asset price fetch job finished in {} ms.", (endTime - startTime));
    }

    /**
     * Finds an asset by its symbol and attempts to fetch and save its latest closing price.
    ...
    private void processAssetLatestPrice(String symbol) {
        ...
        if (assetOptional.isPresent()) {
            ...
            try {
                ...
                logger.error("Failed to fetch price for {}: {}", symbol, e.getMessage(), e);
            }
        } else {
    ...
    /**
     * Executes the API call, parses the JSON response, checks for duplicates, and saves the price if new.
     *
     * IMPORTANT: This method assumes the AssetPriceRepository has a method like:
     * `Optional<AssetPrice> findByAssetAndEffectiveAt(Asset asset, LocalDateTime effectiveAt);`
     * You must ensure this method exists in your AssetPriceRepository interface.
     */
        ...
        if (response.statusCode() == 200) {
            ...
            if (timestampMatcher.find() && closeMatcher.find()) {
                ...
                if (timestamps.length > 0 && prices.length > 0) {
                    // Use the first element as it represents the latest available data point
                    ...
                    if (!tsStr.isEmpty() && !priceStr.isEmpty() && !priceStr.equals("null")) {
                        try {
                            ...
                            if (existingPrice.isPresent()) {
                                logger.warn("Skipped {}: Price for date {} already exists in the database. (ID: {})",
                                        symbol, effectiveAt.toLocalDate(), existingPrice.get().getId());
                                return;
                            }
                            // --- END DUPLICATE CHECK ---
                            ...
                            logger.info("Successfully saved NEW latest price for {}: {} at {}",
                                    symbol, price.toPlainString(), effectiveAt);

                        } catch (NumberFormatException ex) {
                            logger.error("Parsing Error: Invalid number format in API response for {}.", symbol, ex);
                        }
                    } else {
                        logger.info("Skipped {}: Latest data point was null or empty (market closed/data gap).", symbol);
                    }
                } else {
                    logger.error("API response array was empty for {}.", symbol);
                }

            } else {
                logger.error("Could not parse chart data structure for {}. Response body might be incomplete or malformed.", symbol);
            }
