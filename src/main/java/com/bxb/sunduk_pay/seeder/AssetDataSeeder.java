...
/**
 * A comprehensive data seeder responsible for loading all initial static data (Assets, Portfolio Models,
 * and Allocations) and then fetching and persisting historical market prices at application startup.
 * All steps are idempotent, ensuring data integrity upon application restarts.
 */
@Configuration
...
public class AssetDataSeeder {
    ...
    private static final String API_URL_TEMPLATE = "https://apidojo-yahoo" +
            "-finance-v1.p.rapidapi.com/stock/v2/get-chart?interval=1d&range" +
            "=1y&symbol=%s&region=";

    // !!! REPLACE WITH YOUR ACTUAL KEYS !!!
    private static final String API_KEY = "ad73556864msh3a621d64cf35d9ep1b7ccbjsn9591f1b3749b";
    private static final String API_HOST = "apidojo-yahoo-finance-v1.p.rapidapi.com";

    private final HttpClient httpClient = HttpClient.newHttpClient();

    /**
     * Defines the CommandLineRunner to execute all seeding logic sequentially upon application startup.
     * Order of execution: Assets -> Models/Allocations -> Historical Prices.
     *
    ...
    CommandLineRunner loadAllData() {
        return args -> {
            ...
            // 2. Seed Portfolio Models and Allocations (required for core functionality)
    ...
    /**
     * Seeds initial Asset data if the Asset table is empty (Idempotency check).
     */
    private void seedAssets() {
        if (assetRepository.count() == 0) {
            logger.info("Assets table is empty. Seeding initial Asset data...");
            ...
            logger.info("Assets table already contains data. Skipping asset seeding.");
        }
    }

    /**
     * Seeds Portfolio Models and their Allocations if the PortfolioModel table is empty (Idempotency check).
     */
    private void seedPortfolioModelsAndAllocations() {
        if (modelRepository.count() == 0) {
            logger.info("PortfolioModel table is empty. Seeding Models and Allocations...");

            // Fetch assets (They must exist now, thanks to the prior step)
            Asset sukuk = assetRepository.findBySymbol("SPSK")
                    .orElseThrow(() -> new IllegalStateException("SPSK asset not found during model setup."));
            Asset gold = assetRepository.findBySymbol("GLD")
                    .orElseThrow(() -> new IllegalStateException("GLD asset not found during model setup."));
            Asset equity = assetRepository.findBySymbol("HLAL")
                    .orElseThrow(() -> new IllegalStateException("HLAL asset not found during model setup."));
            Asset cash = assetRepository.findBySymbol("AED")
                    .orElseThrow(() -> new IllegalStateException("AED asset not found during model setup."));
            ...
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(lowRisk).asset(sukuk).weight(0.50).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(lowRisk).asset(gold).weight(0.20).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(lowRisk).asset(equity).weight(0.20).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(lowRisk).asset(cash).weight(0.10).build());
            ...
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(mediumRisk).asset(sukuk).weight(0.35).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(mediumRisk).asset(equity).weight(0.40).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(mediumRisk).asset(gold).weight(0.15).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(mediumRisk).asset(cash).weight(0.10).build());
            ...
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(highRisk).asset(equity).weight(0.65).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(highRisk).asset(gold).weight(0.20).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(highRisk).asset(sukuk).weight(0.10).build());
            allocationRepository.save(PortfolioAllocation.builder()
                    .portfolioModel(highRisk).asset(cash).weight(0.05).build());

            logger.info("Successfully seeded 3 PortfolioModels and 12 PortfolioAllocations.");
        } else {
            logger.info("PortfolioModel table already contains data. Skipping model and allocation seeding.");
        }
    }
    ...
    private void seedHistoricalPrices() {
        ...
        for (Map.Entry<String, String> entry : tickerMap.entrySet()) {
            ...
            assetRepository.findBySymbol(symbol).ifPresent(asset -> {
                try {
                    logger.info("Fetching historical chart data for: {}", symbol);
                    fetchAndSaveHistory(asset, symbol);
                    // Sleep to respect Rate Limits
                    TimeUnit.SECONDS.sleep(2);
                } catch (Exception e) {
                    logger.error("Failed to seed history for {}: {}", symbol, e.getMessage(), e);
                }
            });
        }
    ...
    /**
     * Executes the API call to fetch historical data and saves it only if the record
     * does not already exist for the specific asset and effective date.
     *
    ...
    private void fetchAndSaveHistory(Asset asset, String symbol)
        ...
        if (response.statusCode() == 200) {
            ...
            Pattern timestampPattern = Pattern.compile("\"timestamp\"\\s*:\\s*\\[([\\d,\\s]*)\\]");
            Matcher timestampMatcher = timestampPattern.matcher(jsonBody);

            // 2. Extract Close Price Array String
            Pattern closePattern = Pattern.compile("\"close\"\\s*:\\s*\\[([\\d\\.,\\snull]*)\\]");
            ...
            if (timestampMatcher.find() && closeMatcher.find()) {
                ...
                for (int i = 0; i < timestamps.length; i++) {
                    ...
                    String priceStr = (i < prices.length) ? prices[i].trim() : "null";

                    // Skip nulls (markets closed or missing data)
                    if (!tsStr.isEmpty() && !priceStr.isEmpty() && !priceStr.equals("null")) {
                        try {
                            ...
                            Optional<AssetPrice> existingPrice = assetPriceRepository.findByAssetAndEffectiveAt(asset, effectiveAt);
                            ...
                            AssetPrice historicalPrice = AssetPrice.builder()
                                    .asset(asset)
                                    .closePrice(price)
                            ...
                            logger.warn("Parsing Error: Skipped data point for {} at index {} due to invalid number format.", symbol, i);
                        }
                    }
                }
                logger.info("Seeded {} new historical data points ({} skipped) for {}", savedCount, skippedCount, symbol);
            } else {
                logger.error("Could not parse chart data structure for {}. Check API response format.", symbol);
            }

        } else if (response.statusCode() == 429) {
            logger.error("API Rate Limit Exceeded (429) for {}.", symbol);
        } else {
            logger.error("API Error {} for {}. Body: {}", response.statusCode(), symbol, response.body());
        }
    }
}